...
def linear_search_artem(arr, target):
    for num in arr:
        if num == target:
            return True
    return False
...
print("Linear")
start = time.time()
print(linear_search_artem(random_nums, num_to_find))
end = time.time()
print(f"Runtime: {end - start}")


def binary_search_artem(arr, target):
    start = 0
    end = (len(arr) - 1)
...
start = time.time()
random_nums.sort()
print(binary_search_artem(random_nums, num_to_find))
end = time.time()
print(f"Runtime: {end - start}")
